...
async def get_distance(start: models.GridLocation, end: models.GridLocation):
    ...
    return round(math.sqrt(((end_x - start_x + 1) ** 2) + ((end_y - start_y + 1) ** 2)), 4)


async def get_rotation_angle(line_start: models.GridLocation, waypoint: models.GridLocation,
                             line_end: models.GridLocation) -> float:
    (start_x, start_y) = line_start
    (waypoint_x, waypoint_y) = waypoint
    (end_x, end_y) = line_end

    vector_start = np.array([start_x - waypoint_x,
                             start_y - waypoint_y])
    vector_end = np.array([end_x - waypoint_x,
                           end_y - waypoint_y])

    vector_start_unit = vector_start / np.linalg.norm(vector_start)
    vector_end_unit = vector_end / np.linalg.norm(vector_end)
    angle = 180 - abs(float(np.degrees(np.arccos(np.clip(np.dot(vector_start_unit, vector_end_unit), -1.0, 1.0)))))
