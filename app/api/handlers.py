...
class SetGeodataHandler(BaseHandler):
    ...
    async def calculate(self, request_data: dict) \
        ...
        distances = [await direction_finder.get_distance(smoothed[_], smoothed[_ + 1])
    ...
    @staticmethod
    async def to_response(angles: tp.List[float], distances: tp.List[float]) \
